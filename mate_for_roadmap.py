#!/usr/bin/env python
""" 
Code to generate the responses used in the Rogers et al Roadmap paper
Note that leaf scale responses are calculated in Excel - filename GDAY calcs Jul 2015.xls
Was updated in July to ensure that leaf and canopy responses are really comparable
...
if __name__ == "__main__":
    ...
    params.vcmaxnb           = 0.0   
    params.g1                = 2.74 
    params.ctheta_root       = 0.575
    params.ntheta_root       = 6.5
    ...
    # A vs PAR - three levels of LAI, maintain average N per unit leaf area constant and equal to 2.0 g m-2
    Alow_par = np.zeros(N)
    Amed_par = np.zeros(N)
    ...
    for i in xrange(N):
        ...
        Amed_par[i] = M.calculate_photosynthesis(tair=25.0, par=par[i], vpd=1.0, 
                                          ca=380.0, daylen=daylen, sw=1.0)
    ...
    ax.plot(par/2600/daylen, Alow_par, "b-", label="LAI=1")
    ax.plot(par/2600/daylen, Amed_par, "r-", label="LAI=3")
    ax.plot(par/2600/daylen, Ahigh_par, "g-", label="LAI=7")
    ax.set_ylabel('A ($\mu$mol m$^{-2}$ s$^{-1}$)')
    ax.set_xlabel('PAR')
    ax.legend(numpoints=1, loc='best', shadow=True).draw_frame(True)
    plt.show()
    
    # A vs temp - LAI = 3, N content = 6 gN m-2, Ca is 380 or 550
    ...
    for i in xrange(N):
        ...
        Ahigh_tair[i] = M.calculate_photosynthesis(tair=tair[i], par=par, vpd=1.0, 
                                          ca=550.0, daylen=daylen, sw=1.0)
    fig = plt.figure(1)
    ax = fig.add_subplot(111)
    ax.plot(tair, Alow_tair, "b-", label="Ca = 380")
    ax.plot(tair, Ahigh_tair, "r-", label="Ca = 550")
    ax.set_ylabel('A ($\mu$mol m$^{-2}$ s$^{-1}$)')
    ax.set_xlabel('Tair (deg C)')
    ax.legend(numpoints=1, loc='best', shadow=True).draw_frame(True)
    plt.show()
    
    # A vs Ca - LAI = 3, leaf N = 1.5 or 2 
    par = 1500.0 * 3600.0 * daylen    
    Alow_ca = np.zeros(N)
    Ahigh_ca = np.zeros(N)
    for i in xrange(N):
    ...
    # A vs moisture - doing two N levels, but just use the higher one
    par = 1500.0 * 3600.0 * daylen    
    Alow_sw = np.zeros(N)
    Ahigh_sw = np.zeros(N)
    for i in xrange(N):
    ...
    # A vs vpd - doing two N levels, but just use the higher one
    par = 1500.0 * 3600.0 * daylen    
    Alow_vpd = np.zeros(N)
    ...
    f = open("mate_roadmap.csv", "w")
    print >> f, "par tair vpd sw ca alow_par amed_par ahigh_par alow_tair ahigh_tair alow_ca ahigh_ca ahigh_vpd ahigh_sw beta" 
    for i in xrange(N):
        print >> f, par[i], tair[i], vpd[i], sw[i],\
                    ca[i], \
                    Alow_par[i], Amed_par[i], Ahigh_par[i],\
                    Alow_tair[i], Ahigh_tair[i],\
                    Alow_ca[i], Ahigh_ca[i],\
                    Ahigh_vpd[i], Ahigh_sw[i],\
